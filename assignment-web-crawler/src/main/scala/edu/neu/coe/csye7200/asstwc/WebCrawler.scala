...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for(n<-ns\\"a";h<-n\"@href";x<-h.head) yield createURL(Some(u), x.toString())// TO BE IMPLEMENTED
    ...
    // 9 points.
     for (s<-getURLContent(u);res<-MonadOps.asFuture(getLinks(s))) yield res
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    val future = for(usf<-value) yield MonadOps.sequence(usf)
